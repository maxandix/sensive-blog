...
def index(request):
    ...
    #     num_comments=Count('comments', distinct=True)).prefetch_related(
    #     'author').order_by('-num_likes')[:5]
...
def post_detail(request, slug):
    post = Post.objects.prefetch_related('comments__author').get(slug=slug)
    serialized_comments = []
